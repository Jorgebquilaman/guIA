      ...
      <div className="space-y-3">
          {works.map((work) => {
            ...
            return (
              <div
                key={work.id}
                className="flex flex-col gap-3 rounded-xl border border-iupa-light bg-white p-4 shadow-sm transition-shadow hover:shadow-md"
              >
                {/* Fila superior: tipo de documento + año */}
                <div className="flex items-center justify-between gap-3">
                  ...
                  {year && <span className="text-xs text-iupa-medium">{year}</span>}
                </div>

                {/* Título y autores: con todo el ancho disponible */}
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-iupa-medium">
                    ...
                    {work.degreeProgram ? ` — ${work.degreeProgram}` : ''}
                  </p>
                </div>

                {/* Botón ver/descargar al pie */}
                <div className="flex justify-end border-t border-iupa-light pt-3">
                  <a
                    href={`/documentos/${work.id}`}
                    className="rounded-lg border border-iupa-green px-4 py-2 text-xs font-semibold text-iupa-green transition-colors hover:bg-iupa-green hover:text-white"
                  >
