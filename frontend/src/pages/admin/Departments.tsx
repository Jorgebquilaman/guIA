...
export default function Departments() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      addToast('error', 'Error al guardar el departamento')
    }
  }
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('¿Eliminar este departamento? También se eliminarán sus carreras.')) return
    try {
  ...
  return (
    <div className="space-y-8 p-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-iupa-dark">Departamentos y carreras</h1>
          <p className="text-xs text-iupa-medium">Gestioná los departamentos académicos y sus programas</p>
        </div>
      </div>
          ...
          <div className="flex items-center gap-2">
            {editing ? (
              <>
                ...
                <span className="text-sm font-semibold text-iupa-green-secondary">Editar departamento</span>
              </>
            ) : (
              <>
                ...
                <span className="text-sm font-semibold text-iupa-green">Nuevo departamento</span>
              </>
            )}
          </div>
        </div>
        <div className="space-y-5 p-6">
          ...
          <div className="flex justify-end gap-3 border-t border-iupa-light pt-4">
            ...
            >
              ...
              {editing ? 'Actualizar' : 'Crear departamento'}
            </Button>
          </div>
        </div>
      </Card>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-iupa-dark">Departamentos existentes</h2>
        ...
        {departments?.length === 0 ? (
          <Card>
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-sm font-medium text-iupa-dark">No hay departamentos configurados</p>
              <p className="mt-1 text-xs text-iupa-medium">Creá tu primer departamento usando el formulario de arriba</p>
            </div>
          </Card>
        ) : (
          <div className="space-y-3">
                  ...
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <span className="ml-2 rounded-full bg-iupa-light px-2 py-0.5 text-xs font-medium text-iupa-medium">
                        {d.degreePrograms.length} carrera{d.degreePrograms.length !== 1 ? 's' : ''}
                      </span>
                  ...
                  <div className="border-t border-iupa-light bg-iupa-green-light/20 px-5 py-4">
                    {d.degreePrograms.length === 0 ? (
                      <div className="flex items-center gap-2 py-2">
                        ...
                        <p className="text-xs font-medium text-iupa-medium">Sin carreras configuradas</p>
                      </div>
                    ) : (
                      <div className="mb-3 flex flex-wrap gap-2">
                            ...
                            <button
                              onClick={async () => {
                                try {
                                  ...
                                  addToast('success', 'Carrera eliminada')
                                } catch {
                                  addToast('error', 'Error al eliminar carrera')
                                }
                              }}
                          ...
                          onKeyDown={async (e) => {
                            if (e.key === 'Enter' && inlineNewProgram.trim()) {
                              try {
                                ...
                                addToast('success', 'Carrera agregada')
                                setInlineNewProgram('')
                              } catch {
                                addToast('error', 'Error al agregar carrera')
                              }
                            }
                          }}
